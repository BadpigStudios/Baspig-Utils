package baspig.apis.utils.events.block.register.presets;

import baspig.apis.utils.events.block.register.presets.entity.TntEntityPreset;
...
public class TntBlockPreset extends Block {


    public static final BooleanProperty UNSTABLE = Properties.UNSTABLE;


    public TntBlockPreset(Settings settings, ExtraBlockSettings blockSettings) {
        super(settings);
    ...
    public void onDestroyedByExplosion(ServerWorld world, BlockPos pos, Explosion explosion) {
        if (world.getGameRules().getBoolean(GameRules.TNT_EXPLODES)) {
            TntEntityPreset tntEntity = new TntEntityPreset(world, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, explosion.getCausingEntity());
    ...
    private static boolean primeTnt(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (world instanceof ServerWorld serverWorld && serverWorld.getGameRules().getBoolean(GameRules.TNT_EXPLODES)) {
            TntEntityPreset tntEntity = new TntEntityPreset(world, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, igniter);
            world.spawnEntity(tntEntity);
